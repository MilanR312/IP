...
void ip::SecondCountUp(){_SecCounter++;if (_SecCounter >= _maxCount){_SecCounter = 0;}Serial.print("SecontInt  ");Serial.println(_SecCounter);}
...
#pragma endregion sets pin to var
#pragma region pinGet

int ip::pins::get_value::CS1(){ return _CS1; }
...
void ip::initialisation::Sd(){
    ...
    if (!SD.begin(_CS1)) {//check if sd card has initialised
    ...
    if (myFile0) { // test if file opened
      myFile0.println("Year;Month;Day;Hour;Minute;Second;Lamp1;Lamp2;Lamp3;Pomp;Volume;Moisture");  //prints to file;  
...
void ip::Tester::Rtc(){
  if (! rtc0.begin()){//see if rtc is connected
    Serial.println("no rtc found");
    while (1);
  }
  if (! rtc0.isrunning()){//check if it is running
...
void ip::sdCard::Print(){
  ...
  if (sdCard::tuc == _SecCounter){//check if code should run
    sdCard::tuc = sdCard::ttc + _SecCounter;//code to calculate the next time the code should run
    if (sdCard::tuc >= _maxCount){//check if the next time is withing the max count
    ...
    sprintf(sdCard::buffer,"%d;%d;%d;%d;%d;%d;%s;%s;%s;%s;%s;%d",
...
void ip::sdCard::PrintTemp(){
  ...
  if (sdCard::tuctemp == _SecCounter){//check if code should run
    sdCard::tuctemp = sdCard::ttctemp + _SecCounter;//code to calculate the next time the code should run
    if (sdCard::tuctemp >= _maxCount){//check if the next time is withing the max count
      sdCard::tuctemp = sdCard::tuctemp - _maxCount;//calculate the time
    }
    sprintf(sdCard::buffertemp,"%s;%s;%s;%s;%s;%d",
    digitalRead(_mosfet24[0])?"on":"off",
    digitalRead(_mosfet24[1])?"on":"off",
    digitalRead(_mosfet24[2])?"on":"off",
    digitalRead(_mosfet24[3])?"on":"off",
...
void ip::mosfet::turnon::ch0(){digitalWrite(_mosfet24[0], HIGH);/*Serial.println("turning on mosfet 0");*/}
...
float ip::flow::flowCalc(){ // call every second:
...
void ip::flow::Pump(){//redo code better !!!!! brol
if (flow::tuc2 == _SecCounter){
    flow::tuc2 = flow::ttc1 + _SecCounter;
    if (flow::tuc2 >= _maxCount){
      flow::tuc2 = flow::tuc2 - _maxCount;
    }
  if (ip::MoistSensor::dHumidity > -10){//test if humidity is too high
    digitalWrite(_mosfet24[3], HIGH);//turn off pump
  }  
  if (ip::MoistSensor::dHumidity < 10){//test if humidity is too low
